...
## Folder-heart artwork and light screenshots — 2026-09-13

The maintainer subsequently chose the Community listing's white `folder-heart` glyph on its orange-to-purple tile. The canonical SVG reproduces the observed tile geometry and both original Lucide paths; the PNG was rasterized directly from that vector. Actual browser comparisons at small and enlarged sizes passed on light and dark backgrounds. The same PNG is used in the README and sample library, with the pinned Lucide license included in both locations.

This changes presentation assets and the sample's light appearance setting. Product ID, version, commands, data formats, dependencies, and all three published runtime files remain unchanged. `npm run verify` passed formatting, zero-warning lint, all 18 test files / 150 tests, TypeScript, production build, and release checks. The resulting JavaScript retains SHA256 `d3af12bbdcbbb53eac01a9c01f5c9786c662097b55e9088edf178b6fe276a4a7`.

The earlier A2 results in this document describe the [immutable 3.0.0 source assets](https://github.com/t1seo/icon-fairy/tree/2521d51704c26801321d9bb0c0d518dd47837f19/assets). Current generic `assets/icon-fairy-*.png` paths now show the selected folder-heart artwork and light captures. Original A1–A6 files, generation records, and dated QA captures remain unchanged.

All nine current screenshots were captured from actual Obsidian 1.13.7 in a new disposable copy of the sample vault: overview, context menu, library, upload preview, batch import preview, annotation, commands, settings, and mobile layout. Every capture was opened and visually inspected. The mobile image uses official desktop emulation at a 390 × 820 CSS viewport; all seven library images loaded without horizontal overflow. It is not a physical-device test.

Selecting Light in the actual Appearance UI saved `"theme": "moonstone"`, which is now included in the sample. Reading and Live Preview displayed the new mark, the original seven icons and assignments loaded, all three commands appeared, and the existing annotation rendered. Single PNG and three-file PNG/SVG import previews were cancelled without adding duplicates. Reloading retained the appearance, library, assignments, settings, and annotation.

The 17 other previously tracked sample files remain byte-identical; all 20 prepared sample files also match their QA baseline after cleanup. Temporary notes were removed, mobile emulation was disabled, and CLI enablement was restored to its original disabled state. All new QA windows were closed; the user's original window and appearance were preserved.

At 14:50 UTC, the existing Community overview attachment had been replaced through the owner edit form with the new light overview. The public page displayed the folder-heart tile and matching light sample, with **Review Passed / Health Excellent** still present. This presentation update does not create or claim a new release review; the previously verified 3.0.0 review remains attached to its original release commit.
